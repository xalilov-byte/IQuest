...
(function (root) {
  ...
  function create(seed, level) {
    ...
    /* Soat orqaga ketmaydi: qurilma vaqti o'zgarsa (NTP) ham fazalar
       buzilmaydi. Jurnalga qisilgan (clamp) qiymat yoziladi — replay
       aynan shu vaqtlarni oladi. */
    function at(now) {
    ...
    function tap(i, now) {
      ...
      const t = at(now);
      tick(t);   // avval vaqt: bosish qaysi fazaga tushganini aniqlaydi
      ...
      if (target[i]) {
        ...
        picked[i] = 2; outcome = 'bad'; if (!tainted) points += found;
        endRound(false, t);
      }
    }
